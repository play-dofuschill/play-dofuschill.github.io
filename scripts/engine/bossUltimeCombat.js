...
function _withBoss_UltimeCtx(fn) {
    ...
    combat = {
        ...
        isBossUltime:          true,
        enemy:                 s.boss,
        sessionLoot:           s.sessionLoot,
        spellStacks:           s.spellStacks,
        memberMoveIndex:       s.memberMoveIndex,
        savedMembers:          s.savedMembers,
        huppermageLastElement: s.huppermageLastElement,
        critStreak:            s.critStreak            || 0,
        activeMemberIndex:     s.currentTurnMemberIdx  >= 0 ? s.currentTurnMemberIdx : 0,
        memberPassiveState:    s.memberPassiveState    || {},
        memberKillCount:       s.memberKillCount       || {},
        huppermageComboCount:  s.huppermageComboCount  || 0,
        enutrof_air_active:    s.enutrof_air_active    || 0,
        enutrof_terre_active:  s.enutrof_terre_active  || 0,
        enutrof_eau_active:    s.enutrof_eau_active    || 0,
        enutrof_traps:         s.enutrof_traps         || [],
        interceptor:           s.interceptor,
        transposerSlot:        s.transposerSlot,
        memberTimers:          {},
        savedMemberMoveIndex:  s.savedMemberMoveIndex || {},
    }
    try {
        return fn()
    } finally {
        s.critStreak           = combat.critStreak           || 0
        s.memberPassiveState   = combat.memberPassiveState   || {}
        s.memberKillCount      = combat.memberKillCount      || {}
        s.huppermageComboCount = combat.huppermageComboCount || 0
        s.enutrof_air_active   = combat.enutrof_air_active   || 0
        s.enutrof_terre_active = combat.enutrof_terre_active || 0
        s.enutrof_eau_active   = combat.enutrof_eau_active   || 0
        s.enutrof_traps        = combat.enutrof_traps        || []
        s.interceptor          = combat.interceptor
        s.transposerSlot       = combat.transposerSlot
        s.savedMemberMoveIndex = combat.savedMemberMoveIndex || {}
...
function _Boss_UltimeHandleMemberDeath(member) {
    ...
    const memberIdx = Boss_UltimeSession.members.indexOf(member)
    // Nettoyage interception si c'était l'intercepteur qui meurt
    if (Boss_UltimeSession.interceptor === memberIdx) {
        delete Boss_UltimeSession.interceptor
        if (combat?.isBossUltime) delete combat.interceptor
    }
    if (Boss_UltimeSession.transposerSlot === memberIdx) {
        delete Boss_UltimeSession.transposerSlot
        if (combat?.isBossUltime) delete combat.transposerSlot
        if (member.transposition) delete member.transposition
    }
...
// _prevSpellId : ID du sort précédent dans la séquence (pour l'effet 'repeat')
async function _Boss_UltimeCastSpell(member, spellId, _summonDepth, _prevSpellId) {
    const s  = Boss_UltimeSession
    const mv = move[spellId]
    if (!mv) return
    ...
    // getEffectiveStats doit être appelé dans le contexte Boss Ultime pour que
    // state.team et combat.memberPassiveState soient correctement swappés.
    let memberStats = member._stats ?? {}
    _withBoss_UltimeCtx(() => { memberStats = getEffectiveStats(member) ?? member._stats ?? {} })
    ...
    // Crit pré-tiré une fois par sort (comme dans le moteur normal)
    const isCritRoll = Math.random() * 100 < (memberStats?.critChance || 0)

    // Sort précédent résolu (pour l'effet 'repeat')
    const _prevRaw = _prevSpellId ? move[_prevSpellId] : null
    const prevMv   = _prevRaw ? applyProgression(_prevRaw, effectiveLvl) : null
    ...
    if (!_summonDepth) {
        ...
        // Tour du compagnon actif (s'il existe et si le membre n'est pas une invocation)
        if (!member.isSummon && member.companion && member.companion.currentHp > 0) {
            if (!s.ended) {
                _withBoss_UltimeCtx(() => {
                    actCompanion(member)
                    if (s.boss.currentHp <= 0 && !s.ended) Boss_UltimeEndCombat('victory')
                })
            }
        }
    }
...
async function Boss_UltimeExecuteMemberTurn(memberIdx, spellIds) {
    ...
    for (let _ci = 0; _ci < spellIds.length; _ci++) {
        ...
        if (typeof updateBoss_UltimeCombatUI === 'function') updateBoss_UltimeCombatUI()
        await _Boss_UltimeCastSpell(member, spellIds[_ci], undefined, _prevSpellId)
        _prevSpellId = spellIds[_ci]
    }
    s.currentCastIdx    = -1
    s.currentTurnSpells = []
    if (s.ended) return
...
// Retourne une copie de l'effet avec ses dommages (damage + dot) scalés par le facteur donné.
// Les debuffs/buffs ne sont pas scalés : seul le volume de dégâts est affecté.
function _Boss_UltimeScaleEffect(effect, scale) {
    if (scale === 1.0) return effect
    const copy = { ...effect }
    if (copy.type === 'damage' && copy.damage) {
        copy.damage = {
            min: Math.max(0, Math.round(copy.damage.min * scale)),
            max: Math.max(0, Math.round(copy.damage.max * scale))
        }
    }
    if (copy.type === 'dot' && copy.value != null) {
        copy.value = Math.max(0, Math.round(copy.value * scale))
    }
    return copy
}
...
async function _Boss_UltimeBossAttack() {
    ...
    const isCritRoll = Math.random() * 100 < (bossStats.critChance || 0)

    // Interception : si un membre a posé 'interception', toutes les attaques du boss le ciblent à 100%
    const interceptorIdx = s.interceptor !== undefined ? s.interceptor : -1

    // 100% des dégâts sur le membre qui vient de jouer (ou l'intercepteur), 50% sur les deux autres
    for (const effect of (mv.effects || [])) {
        // Applique l'effet à chaque membre vivant
        for (let mi = 0; mi < s.members.length; mi++) {
            const member = s.members[mi]
            if (!member || member.currentHp <= 0) continue

            // Effets qui ciblent 'self' (le boss lui-même) : une seule fois
            if (effect.target === 'self') {
                const _savedTurnIdx = s.currentTurnMemberIdx
                s.currentTurnMemberIdx = mi
                _withBoss_UltimeCtx(() => {
                ...
                s.currentTurnMemberIdx = _savedTurnIdx
                break   // 'self' : une seule exécution suffit
            }

            // Interception active : seul l'intercepteur reçoit les coups (à 100%)
            if (interceptorIdx !== -1 && mi !== interceptorIdx) continue

            const isMainTarget = interceptorIdx !== -1
                ? true
                : (s.currentTurnMemberIdx >= 0 && mi === s.currentTurnMemberIdx)
            const scaledEffect = _Boss_UltimeScaleEffect(effect, isMainTarget ? 1.0 : 0.5)

            const _bPreLen = s.log.length
            const _bName   = member.name || classes[member.classId]?.name || '?'

            // Fix activeMemberIndex + getEffectiveStats : on cible temporairement ce membre
            // pour que _applyItemOnEffectTriggers et getEffectiveStats utilisent le bon slot.
            const _savedTurnIdx = s.currentTurnMemberIdx
            s.currentTurnMemberIdx = mi
            let memberStats = member._stats ?? {}
            _withBoss_UltimeCtx(() => {
                memberStats = getEffectiveStats(member) ?? member._stats ?? {}
                executeEffect({
                    caster:      s.boss,
                    casterStats: bossStats,
                    targetEnemy: member,
...
function _Boss_UltimeOnVictory() {
    ...
    // Récompense item
    if (dragon.reward?.item) {
        addToInventory(dragon.reward.item)
        const itm = item[dragon.reward.item]
        showNotification(`${itm?.name || dragon.reward.item} obtenu ! — ${s.boss.name} vaincu !`, 'success')
    }

    // Première victoire : débloquage de classe
    if (isFirst) {
        ds.firstVictory = true
        if (s.dragonId === 'dragon_ignemikhal') {
    ...
    // Reset le dragon (PV max, phase 1)
...
function Boss_UltimeStartCombat(dragonId) {
    ...
    // Initialisation des états passifs par membre (comme startCombat dans combat.js)
    const _initPassiveState = {}
    const ROULETTE = [
    ...
    const _DYNAMIC_HUPPA = new Set(['best_element_damage', 'worst_element_damage',
    ...
    for (let i = 0; i < members.length; i++) {
        ...
        _initPassiveState[i] = {}
        if (passive?.id === 'pandawa') _initPassiveState[i].state = 'normal'
        if (passive?.id === 'ecaflip') {
            _initPassiveState[i].ecaflipEffect = ROULETTE[Math.floor(Math.random() * ROULETTE.length)]
        }
        if (passive?.id === 'huppermage' && m.moves) {
            const _slots = ['slot1', 'slot2', 'slot3', 'slot4']
            const _elems = _slots.map(sl => {
                const mid = m.moves?.[sl]
                if (!mid) return null
                const mv = move[mid]
                if (!mv) return null
                if (mv.effects?.some(e => _DYNAMIC_HUPPA.has(e.type))) return null
                const dmgElems = new Set(
                    (mv.effects || []).filter(e => e.type === 'damage' || e.type === 'dot' || e.type === 'damage_zone')
                        .map(e => e.element).filter(e => e && e !== 'neutre')
                )
                return dmgElems.size === 1 ? [...dmgElems][0] : null
            }).filter(Boolean)
            _initPassiveState[i].huppermageBonus = _elems.length === 4 && new Set(_elems).size === 4
        }
    }
    ...
    Boss_UltimeSession = {
        ...
        bossCastingIdx:       -1,
        bossCastDuration:     0,
        // Champs synchronisés avec le faux objet combat dans _withBoss_UltimeCtx
        memberPassiveState:   _initPassiveState,
        memberKillCount:      {},
